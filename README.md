...
| Дата и время | Первый запуск для `once` и `after_completion`, в часовом поясе HA. |
...
каждую вторую неделю. Якорная неделя сохраняется вместе с напоминанием, поэтому
расписание не сбивается после перезапуска и учитывает DST часового пояса HA.
...
|---|---|---|
| `sensor` | `<название> status` | Состояние `scheduled`, `active` или `snoozed`; атрибуты содержат ID, тип, следующий запуск, получателей и интервалы. |
...
| `smart_reminder_repeated` | Повтор до выполнения или срабатывание после snooze. |
| `smart_reminder_snoozed` | Напоминание отложено. |
| `smart_reminder_completed` | Напоминание выполнено; для recurring уже рассчитан следующий запуск. |
...
Поле `text` completed-события может быть пустой строкой.
...
conditions:
  - condition: template
    value_template: "{{ trigger.event.data.recipient_ids | count > 0 }}"
actions:
  - variables:
  ...
  - choose:
      ...
      - conditions:
          - condition: template
            value_template: >-
              {{ trigger.id == 'completed' and reminder.text | length > 0 }}
        sequence:
          - action: telegram_bot.send_message
            data:
              chat_id: "{{ reminder.recipient_ids | map('int') | list }}"
...
- **Cron запускается не в то время:** cron вычисляется в часовом поясе,
  выбранном в общих настройках Home Assistant.
